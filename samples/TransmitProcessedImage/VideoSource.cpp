...
{
    if ( mConnectionID.size() <= 0 )
    {
        // No device address, prompts user
        PvDeviceFinderWnd lWnd;
        if ( lWnd.ShowModal().IsOK() )
        {
            mConnectionID = lWnd.GetSelected()->GetConnectionID();
        }
    }

    // Connect device
    PvResult lResult;
    mDevice = PvDevice::CreateAndConnect( mConnectionID.c_str(), &lResult );
    if ( !lResult.IsOK() )
    {
    ...
    // Open stream
    mStream = PvStream::CreateAndOpen( mConnectionID.c_str(), &lResult );
    if ( !lResult.IsOK() )
    {
